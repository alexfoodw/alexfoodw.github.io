...
Now, when comparing the results generated by our method with our baseline model, we observe that our method is able to better respect the relationships between objects. This can be seen in the scene graph input with _sheep by sheep_ (third set of images from the left), where our method clearly illustrates two sheep beside each other, while we only clearly see one sheep in the image generated by our baseline model. 
